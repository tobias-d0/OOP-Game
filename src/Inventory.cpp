...
void Inventory::removeItem(Item* item) { //logic for removing item type
    for (int i = 0; i < MAX_SIZE; i++) {
        if (items[i] == item) {
            items[i] = nullptr;
            currentSize--;
...
void Inventory::removeItem(int index) { //removing item by item in inventory place
...
void Inventory::removeItem(std::string itemName) { //remove item by item name
    for (int i = 0; i < MAX_SIZE; i++) {
